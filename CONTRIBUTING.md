# Contributing to OpenDoist

Thanks for helping build OpenDoist. This is a solo-maintained project with a written spec; small focused PRs that follow the conventions below merge fastest.
...
pnpm --filter @opendoist/core test   # core only: golden tables + property tests
pnpm --filter @opendoist/web dev     # token showcase on Vite dev server
```
...
The component rules below are copied verbatim from the research dossier (§2.9, `docs/superpowers/research/2026-07-15-opendoist-research.md`) and are the binding contract for component PRs. **Any deviation must edit this table in the same PR** — silent divergence from the cheatsheet is a bug, not a judgment call.
...
OpenDoist is [AGPL-3.0](LICENSE). By contributing you agree to license your work under the same terms.
