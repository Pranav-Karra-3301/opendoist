...
(`opendoist.db`), your `attachments/`, nightly `backups/`, and the
...
OpenDoist takes a nightly snapshot with SQLite's `VACUUM INTO` and zips it (with
...
Screen**, and only on **iOS/iPadOS 16.4 or newer**. In Safari, open OpenDoist,
tap the Share button, choose **Add to Home Screen**, then open OpenDoist from the
new icon and enable notifications there. (OpenDoist shows these steps in an
...
That's Google, not OpenDoist. Google Calendar refreshes subscribed (`webcal`/
...
OpenDoist stores priority the intuitive way: **1 = p1 (highest) … 4 = p4
...
you [import from Todoist](import-todoist.md), OpenDoist maps the values for you,
...
automatically. To create another account, set `OPENDOIST_ALLOW_REGISTRATION=true`
...
Set `OPENDOIST_DISABLE_UPDATE_CHECK=true` and restart. OpenDoist otherwise polls
...
same machine. Terminate TLS at a reverse proxy and set `OPENDOIST_PUBLIC_URL` —
see [Running behind a reverse proxy](install.md#running-behind-a-reverse-proxy).

## What's explicitly out of scope?

OpenDoist is single-user and deliberately narrow. Not planned for v1:
