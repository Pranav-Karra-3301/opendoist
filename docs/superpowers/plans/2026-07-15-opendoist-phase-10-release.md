...
- [ ] **Step 4:** Output: one line per entity group, final line `seed: done — 4 projects, 5 labels, 2 filters, 19 open tasks, 6 completed`. `--verify` flag re-opens the DB and asserts those counts, exiting non-zero on mismatch.
