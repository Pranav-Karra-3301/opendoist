# OpenDoist Documentation

OpenDoist is a self-hosted, single-user, keyboard-first task manager — an open,
...
| [Configuration](configuration.md) | Every `OPENDOIST_*` environment variable, its default, and what it does. |
...
| [Command-line client](cli.md) | Installing `opendoist`, logging in, the command table, and `--json` output for scripting. |
