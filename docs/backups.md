...
OpenDoist backs itself up automatically every night and lets you take on-demand
...
Each backup is one zip under `<OPENDOIST_DATA_DIR>/backups/`:

| Entry            | Contents                                                                              |
| ---------------- | ------------------------------------------------------------------------------------- |
| `opendoist.db`   | A `VACUUM INTO` snapshot of the live SQLite database — a single, consistent file.     |
| `meta.json`      | `{ app: "opendoist", version, createdAt, includesAttachments, schema: "v1" }`.        |
...
configurable (see `OPENDOIST_BACKUP_CRON`).

## Configuration

All backup behavior is controlled by environment variables (prefix
`OPENDOIST_`). Per-instance overrides for retention and attachment inclusion can
...
| -------------------------------------- | ----------- | ----------------------------------------------------------------------------------- |
| `OPENDOIST_DATA_DIR`                   | `/data`     | Root data directory; backups live in `<dir>/backups/`.                              |
| `OPENDOIST_BACKUP_CRON`                | `0 3 * * *` | Cron expression for the nightly backup (5-field, server local time).               |
| `OPENDOIST_BACKUP_RETENTION`           | `14`        | How many scheduled + manual backups to keep (count-based — see below). Min `1`.     |
| `OPENDOIST_BACKUP_INCLUDE_ATTACHMENTS` | `true`      | Whether backups embed the `attachments/` tree. `1`/`true`/`yes` = on.               |
...
(`/^opendoist-(backup|prerestore)-\d{4}-\d{2}-\d{2}(-\d{6})?\.zip$/`) that also
guards the download route against path traversal.

- **Scheduled / manual** — `opendoist-backup-YYYY-MM-DD.zip` (UTC date). If that
  name is already taken (a second backup the same day), a UTC time suffix is
  appended: `opendoist-backup-YYYY-MM-DD-HHMMSS.zip`.
- **Pre-restore safety snapshots** — always timestamped:
  `opendoist-prerestore-YYYY-MM-DD-HHMMSS.zip`.
...
1. The uploaded zip is opened and its `opendoist.db` extracted to a temp dir.
...
3. The lock is acquired and a **pre-restore safety snapshot**
   (`opendoist-prerestore-…zip`) is taken first.
4. The live `opendoist.db` (plus any `-wal` / `-shm`) is moved aside, the
...
docker compose stop opendoist

# 2. Unzip the backup somewhere temporary
unzip opendoist-backup-2026-07-15.zip -d /tmp/restore

# 3. Replace the live database inside the data volume.
#    Delete any stale WAL/SHM sidecars so SQLite doesn't reapply them.
docker run --rm -v opendoist-data:/data -v /tmp/restore:/restore alpine sh -c '
  rm -f /data/opendoist.db /data/opendoist.db-wal /data/opendoist.db-shm &&
  cp /restore/opendoist.db /data/opendoist.db &&
...
docker compose start opendoist
```

If you run OpenDoist directly (not in Docker), do the same three file operations
against `$OPENDOIST_DATA_DIR` while the process is stopped.
...
> `opendoist.db` **file**, not the `attachments/` tree, so keep the built-in zip
...
```yaml
dbs:
  - path: /data/opendoist.db
    replicas:
      - type: s3
        bucket: my-opendoist-backups
        path: opendoist
        region: us-east-1
```
...
```sh
litestream restore -o /data/opendoist.db s3://my-opendoist-backups/opendoist
```
