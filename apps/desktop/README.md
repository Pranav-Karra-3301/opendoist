...
The main window is the full OpenTask web app; the desktop build simply talks to your instance over a bearer token instead of a browser cookie (which also sidesteps CORS).

---

## Install

1. Download **`OpenTask_<version>_universal.dmg`** from the latest [desktop release](https://github.com/Pranav-Karra-3301/opentask/releases). The build is *universal* — one download runs natively on both Apple Silicon and Intel Macs.
2. Open the `.dmg` and drag **OpenTask** into your **Applications** folder.
3. **First launch is blocked by Gatekeeper.** The app is *ad-hoc signed but not notarized* (see [below](#why-does-macos-say-the-app-is-unverified)), so macOS refuses it the first time:
   ...
   - Scroll to the security section — you'll see *"OpenTask" was blocked to protect your Mac.*
   ...
   >
   > ```sh
   > xattr -dr com.apple.quarantine /Applications/OpenTask.app
   > ```
...
1. **Instance URL** — the full `https://` URL of your OpenTask server (HTTP is rejected; the token would travel in the clear otherwise).
2. **API token** — an `ot_…` token. Mint one in your instance under **Settings → Integrations**.
...
OpenTask is distributed **without an Apple Developer account**, so the app is **ad-hoc signed** (`signingIdentity: "-"`) rather than signed with an Apple certificate and **notarized**. Practically:
...
pnpm --filter @opentask/desktop tauri dev

# Production bundle (.app + .dmg) under apps/desktop/src-tauri/target/…/bundle/.
pnpm --filter @opentask/desktop tauri build
```
...
>
> ```sh
> pnpm --filter @opentask/desktop tauri build --debug
> open apps/desktop/src-tauri/target/debug/bundle/macos/OpenTask.app
> ```
...
- The **private key** and its password are **never committed**. They live at `~/.tauri/opentask_updater.key` (+ `.password.txt`) and as the GitHub Actions secrets **`TAURI_SIGNING_PRIVATE_KEY`** and **`TAURI_SIGNING_PRIVATE_KEY_PASSWORD`**, which the workflow passes to the Tauri CLI.
...
```sh
pnpm --filter @opentask/desktop exec tauri signer generate -w ~/.tauri/opentask_updater.key
# then paste the printed public key into plugins.updater.pubkey, and update the secrets:
gh secret set TAURI_SIGNING_PRIVATE_KEY          < ~/.tauri/opentask_updater.key
gh secret set TAURI_SIGNING_PRIVATE_KEY_PASSWORD < ~/.tauri/opentask_updater.password.txt
```
...
```
https://github.com/Pranav-Karra-3301/opentask/releases/latest/download/latest.json
```
