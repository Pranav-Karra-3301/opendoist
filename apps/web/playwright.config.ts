...
/**
 * E2E config for the OpenTask web shell. A `setup` project registers the first-run owner and
 ...
 *    `OPENTASK_DATA_DIR=./data`, which would clobber the outer env var and reuse a persistent DB
 *    (breaking first-run registration on the second run). `start` sets no data dir of its own.
 *  - `OPENTASK_DATA_DIR="$(mktemp -d)"` gives every run a fresh SQLite DB (auto-migrated on boot),
 *    so `owner@example.com` can always be registered as the first account.
 *  - `OPENTASK_PUBLIC_URL=http://127.0.0.1:5173` makes the API's public origin the web origin the
...
export default defineConfig({
  ...
  webServer: [
    {
      command:
        // OPENTASK_DISABLE_UPDATE_CHECK: the phase-9 boot-time update.check job would otherwise
        // call the real GitHub releases API on every suite run (external network, nondeterministic).
        'OPENTASK_DATA_DIR="$(mktemp -d)" OPENTASK_PUBLIC_URL=http://127.0.0.1:5173 OPENTASK_DISABLE_UPDATE_CHECK=true pnpm --filter @opentask/server start',
    ...
    },
    {
      ...
      command: 'pnpm --filter @opentask/web exec vite --host 127.0.0.1 --port 5173 --strictPort',
